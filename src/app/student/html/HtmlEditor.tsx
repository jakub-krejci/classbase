...
interface WebFile {
    path: string; // full path to storage
...
function storagePath(
    ...
    return folder
        ? `zaci/${uid}/${projKey}/${folder}/${name}`
        : `zaci/${uid}/${projKey}/${name}`;
}
...
function buildPreview(files: WebFile[], contents: Map<string, string>): string {
    const htmlFile =
        files.find((f) => f.name === "index.html" && f.folder === "") ??
        files.find((f) => f.type === "html");
    if (!htmlFile)
        return '<p style="font-family:sans-serif;padding:20px;color:#888">Žádný HTML soubor v projektu</p>';
...
export default function HtmlEditor({ profile }: { profile: any }) {
    ...
    const previewDragRef = useRef<{ startY: number; startH: number } | null>(
        null,
    );
    ...
    const [deleteProjModal, setDeleteProjModal] = useState<WebProject | null>(
        null,
    );
    const [renameProjModal, setRenameProjModal] = useState<WebProject | null>(
        null,
    );
    const [renameProjVal, setRenameProjVal] = useState("");
    const [openProjModal, setOpenProjModal] = useState(false);
    const [newItemModal, setNewItemModal] = useState<{ folder: string } | null>(
        null,
    );
    ...
    async function pushFile(
        ...
        let { error } = await supabase.storage
            .from(BUCKET)
            .upload(path, blob, { upsert: true, cacheControl: "0" });
        if (error) {
            await supabase.storage.from(BUCKET).remove([path]);
            const r2 = await supabase.storage
                .from(BUCKET)
                .upload(path, blob, { cacheControl: "0" });
    ...
    async function fetchAsDataUrl(path: string): Promise<string> {
        ...
        const correctBlob =
            mime !== data.type ? new Blob([data], { type: mime }) : data;
    ...
    const refreshProjects = useCallback(async (): Promise<WebProject[]> => {
        ...
        for (const item of top) {
            ...
            result.push({
                ...
                key: item.name,
                files,
                updatedAt: files[0]?.updatedAt ?? new Date().toISOString(),
            });
        }
        setProjects(result);
        setLoadingProj(false);
        ...
        for (const item of data) {
            if (item.metadata === null || item.metadata === undefined) {
                ...
                if (!item.name.includes(".")) {
                    await listFolder(
                        `${storagePfx}/${item.name}`,
                        folderRelative
                            ? `${folderRelative}/${item.name}`
                            : item.name,
                        projKey,
                        out,
                    );
            ...
            w.require(["vs/editor/editor.main"], (monaco: any) => {
                ...
                if (editorContainerRef.current) {
                    ...
                    editorRef.current.addCommand(
                        ...
                        () => document.getElementById("html-save-btn")?.click(),
                    );
                }
                setMonacoReady(true);
            });
    ...
    useEffect(() => {
        ...
        if (splitView && splitContainerRef.current && !splitEditorRef.current) {
            ...
            splitEditorRef.current = monaco.editor.create(
                splitContainerRef.current,
                {
                    ...
                    scrollbar: { horizontal: "auto", vertical: "auto" },
                    value: splitFile
                        ? (contents.get(splitFile.path) ?? "")
                        : "",
                    language: splitFile
                        ? getMonacoLanguage(splitFile.type)
                        : "html",
                },
            );
    ...
    function openInSplit(file: WebFile) {
        ...
        try {
            localStorage.setItem(LS_LAST2, file.path);
        } catch {}
    }
    ...
    async function openProject(proj: WebProject) {
        ...
        if (firstHtml) {
            ...
            if (monaco && editorRef.current) {
                editorRef.current.setModel(
                    monaco.editor.createModel(content, "html"),
                );
            }
        ...
        if (splitView && lastSplit) {
            ...
            if (sf) {
                ...
                const monaco = monacoRef.current;
                if (monaco && splitEditorRef.current)
                    splitEditorRef.current.setModel(
                        monaco.editor.createModel(
                            sc,
                            getMonacoLanguage(sf.type),
                        ),
                    );
                setSplitFile(sf);
            }
        ...
        setRecent((prev) => {
            const n = [entry, ...prev.filter((r) => r.key !== proj.key)].slice(
                0,
                3,
            );
            try {
                localStorage.setItem(LS_RECENT, JSON.stringify(n));
            } catch {}
            return n;
        });
        try {
            localStorage.setItem(LS_LAST, proj.key);
        } catch {}
    }
    ...
    function updatePreview(c?: Map<string, string>, files?: WebFile[]) {
        ...
        if (previewRef.current)
            previewRef.current.srcdoc = buildPreview(ff, cc);
    }
    useEffect(() => {
        if (monacoReady && activeProject) updatePreview();
    }, [monacoReady]);
        ...
        await Promise.all(
            activeProject.files
                ...
                .map(async (f) => {
                    ...
                    if (!c) return;
                    const e = await pushFile(f.path, c);
                    if (e) errs.push(e);
                }),
        );
    ...
    async function doDeleteProject(proj: WebProject) {
        ...
        await supabase.storage
            .from(BUCKET)
            .remove(proj.files.map((f) => f.path));
        const projs = await refreshProjects();
        ...
        await Promise.all(
            proj.files.map(async (f) => {
                ...
                const content =
                    f.type === "img"
                        ? null
                        : (contents.get(f.path) ?? (await fetchText(f.path)));
            ...
            const content =
                ext === "html"
                    ...
                    : ext === "css"
                      ...
                      : DEFAULT_JS;
            await pushFile(
                storagePath(uid, activeProject.key, folder, fileName),
                content,
            );
        }
        const projs = await refreshProjects();
    ...
    async function doRenameFile(file: WebFile) {
        ...
        if (file.type === "img") {
            ...
            const content =
                contents.get(file.path) ?? (await fetchText(file.path));
            await pushFile(newPath, content);
            // Update contents cache
            setContents((prev) => {
                const n = new Map(prev);
                n.set(newPath, content);
                n.delete(file.path);
                return n;
            });
            contentsRef.current.set(newPath, content);
            contentsRef.current.delete(file.path);
        }
        ...
        if (activeFile?.path === file.path) {
            ...
            if (newFile) {
                ...
                editorFilePath.current = newPath;
            }
        }
        setRenameModal(null);
        setSaving(false);
    ...
    async function uploadImages(files: FileList) {
        ...
        for (const file of Array.from(files)) {
            ...
            if (dataUrl) {
                contentsRef.current.set(path, dataUrl);
                setContents((prev) => {
                    const n = new Map(prev);
                    n.set(path, dataUrl);
                    return n;
        ...
        flash(
            `✓ ${files.length === 1 ? "1 obrázek nahrán" : files.length + " obrázky nahrány"}`,
        );
        setUploadingImg(false);
    }
    ...
    async function dropFileOnFolder(file: WebFile, targetFolder: string) {
        ...
        if (file.type === "img") {
            ...
            if (imgData) {
                ...
                if (dataUrl) {
                    contentsRef.current.set(newPath, dataUrl);
                    contentsRef.current.delete(file.path);
                }
            }
        } else {
    ...
    async function downloadProject() {
        ...
        if (!w.JSZip) {
            try {
                ...
                flash("❌ JSZip chyba: " + (e?.message ?? ""));
                return;
            }
        }
        if (!w.JSZip) {
            flash("❌ JSZip nedostupný");
            return;
        }
        const zip = new w.JSZip();
        for (const file of activeProject.files.filter(
            (f) => f.name !== ".gitkeep",
        )) {
            if (file.type === "img") {
                const { data } = await supabase.storage
            ...
            <div style={{ display: "flex", gap: 8 }}>
                <button
                    ...
                    style={{
                        flex: 1,
                        padding: "10px",
                        background: danger ? D.danger : accent,
                        color: "#fff",
                        border: "none",
                        borderRadius: 9,
                        fontSize: 13,
    ...
    const folders = activeProject
        ? [...new Set(activeProject.files.map((f) => f.folder))].sort()
        : [];
            ...
            const r = Math.min(
                ...
                Math.max(
                    ...
                    splitDragRef.current.startRatio +
                        ((ev.clientX - splitDragRef.current.startX) / areaW) *
                            100,
                ),
            );
            setSplitRatio(r);
        ...
        const mv = (ev: MouseEvent) => {
            ...
            setPreviewHeight(
                Math.min(
                    ...
                    Math.max(
                        100,
                        previewDragRef.current.startH +
                            (previewDragRef.current.startY - ev.clientY),
                    ),
                ),
            );
        };
        ...
        <DarkLayout profile={profile} activeRoute="/student/html" fullContent>
            ...
            {newProjModal && (
                ...
                >
                    ...
                    />
                    <MBtns
                        onOk={() => doCreateProject(newProjName)}
            ...
            {deleteProjModal && (
                <Modal
                    ...
                    onClose={() => setDeleteProjModal(null)}
                >
                    <p
                        style={{
                            fontSize: 13,
                    ...
                    >
                        Smazat{" "}
                        <strong style={{ color: D.txtPri }}>
                            {deleteProjModal.name}
                        </strong>{" "}
                        a všechny soubory?
                    </p>
                    <p
                    ...
                    <MBtns
                        onOk={() => doDeleteProject(deleteProjModal)}
                        onCancel={() => setDeleteProjModal(null)}
                        label="Smazat"
                        danger
                    />
                </Modal>
            ...
            {renameProjModal && (
                <Modal
                    ...
                    onClose={() => setRenameProjModal(null)}
                >
                    <input
                        value={renameProjVal}
                        ...
                        onKeyDown={(e) =>
                            e.key === "Enter" &&
                            renameProjVal.trim() &&
                            doRenameProject(renameProjModal)
                        }
                        autoFocus
                        placeholder={renameProjModal.name}
                        style={{ ...modalInp, marginBottom: 14 }}
                    />
                    <MBtns
                        onOk={() => doRenameProject(renameProjModal)}
                        onCancel={() => setRenameProjModal(null)}
                        label="Přejmenovat"
                        disabled={!renameProjVal.trim()}
                    />
                </Modal>
            )}
            {openProjModal && (
                <Modal
                    ...
                    onClose={() => setOpenProjModal(false)}
                >
                    <div
                        style={{
                            maxHeight: 360,
                    ...
                    <button
                        onClick={() => setOpenProjModal(false)}
                        style={{
                            width: "100%",
                            padding: "10px",
                            background: D.bgMid,
            ...
            {newItemModal !== null && (
                ...
                >
                    ...
                    <input
                        ...
                        onKeyDown={(e) => e.key === "Enter" && doNewItem()}
                        autoFocus
                        placeholder={
                            newItemType === "folder"
                                ? "nazev_slozky"
                ...
                >
                    <input
                        ...
                        onKeyDown={(e) =>
                            e.key === "Enter" &&
                            renameVal.trim() &&
                            doRenameFile(renameModal)
                        }
                        autoFocus
                        placeholder={renameModal.name}
                        style={{ ...modalInp, marginBottom: 14 }}
                    />
                    <MBtns
                        onOk={() => doRenameFile(renameModal)}
            ...
            {splitPickOpen && (
                <Modal
                    title="📄 Otevřít ve druhém editoru"
                ...
                >
                    ...
                    >
                        {activeProject?.files
                            .filter(
                                (f) =>
                                    f.type !== "img" && f.name !== ".gitkeep",
                            )
                            .map((f) => (
                                <div
                                    key={f.path}
                                    onClick={() => openInSplit(f)}
                                    style={{
                                        display: "flex",
                                        alignItems: "center",
                                        gap: 8,
      ...
      `}</style>
            ...
            >
                    ...
                    >
                        ...
                        >
                            ...
                            <button
                                ...
                                style={{
                                    ...sideBtn,
                                    opacity: !activeProject || saving ? 0.4 : 1,
                                }}
                                disabled={!activeProject || saving}
                                onClick={saveAll}
                            >
                                <span>💾</span> Uložit vše
                            </button>
                            <button
                                className="w-sb"
                                style={{
                                    ...sideBtn,
                                    opacity: !activeProject ? 0.4 : 1,
                                }}
                                disabled={!activeProject}
                                onClick={() => imgInputRef.current?.click()}
                            >
                                <span>🖼</span>{" "}
                                {uploadingImg ? "Nahrávám…" : "Nahrát obrázky"}
                            </button>
                        </div>
                    </div>
                        ...
                        >
                            ...
                            {activeProject && (
                                <div style={{ display: "flex", gap: 3 }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            setRenameProjModal(activeProject);
                                            setRenameProjVal(
                                                activeProject.name,
                                            );
                                        }}
                                    ...
                                    <button
                                        onClick={() =>
                                            setDeleteProjModal(activeProject)
                                        }
                                        style={{
                                            padding: "1px 5px",
                                            background: "none",
                                                    ...
                                                    style={{
                                                        ...
                                                        background:
                                                            ...
                                                            activeFile?.path
                                                                ? accent + "18"
                                                                : "transparent",
                                                        border: `1px solid ${f.path === activeFile?.path ? accent + "30" : "transparent"}`,
                                                    }}
                                                    onClick={() => clickFile(f)}
                                                >
                                                    <img
                ...
                >
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: splitView
                                    ? accent + "20"
                                    : "rgba(255,255,255,.04)",
                                color: splitView ? accent : D.txtSec,
                                border: `1px solid ${splitView ? accent + "40" : D.border}`,
                                borderRadius: 7,
                                fontSize: 11,
                                cursor: "pointer",
                                fontFamily: "inherit",
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: livePreview
                                    ? D.success + "18"
                                    : "rgba(255,255,255,.04)",
                                color: livePreview ? D.success : D.txtSec,
                                border: `1px solid ${livePreview ? D.success + "40" : D.border}`,
                                borderRadius: 7,
                                fontSize: 11,
                                cursor: "pointer",
                                fontFamily: "inherit",
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: isDirty
                                    ? accent + "20"
                                    : "rgba(255,255,255,.04)",
                                color: isDirty ? accent : D.txtSec,
                                border: `1px solid ${isDirty ? accent + "40" : D.border}`,
                                borderRadius: 7,
                                fontSize: 12,
                                fontWeight: 600,
                                cursor: "pointer",
                                fontFamily: "inherit",
                                transition: "all .2s",
                                opacity: !activeFile || saving ? 0.4 : 1,
                            }}
                        >
                            {saving ? "…" : "💾 Uložit"}
                        </button>
                    ...
                    >
                        <div
                            style={{
                                display: "flex",
                                flexDirection: "column",
                                width: splitView ? `${splitRatio}%` : "100%",
                                flexShrink: 0,
                                overflow: "hidden",
                            }}
                        >
                            {activeFile?.type === "img" ? (
                                ...
                                >
                                    {contents.get(activeFile.path) ? (
                                        <img
                                            src={contents.get(activeFile.path)}
                                            alt={activeFile.name}
                                            style={{
                                                maxWidth: "80%",
                                                maxHeight: 300,
                    ...
                    <div
                        onMouseDown={onPreviewDividerDown}
                    ...
                    >
                        ...
                        >
                                        ...
                                        style={{
                                            ...
                                            color:
                                                previewWidth === w
                                                    ? accent
                                                    : D.txtSec,
                                            border: `1px solid ${previewWidth === w ? accent + "40" : D.border}`,
                                            borderRadius: 5,
                                            fontSize: 11,
                                            cursor: "pointer",
                                            fontFamily: "inherit",
                            ...
                            >
                                {activeProject
                                    ? `${activeProject.name}${previewWidth ? ` — ${previewWidth}px` : ""}`
                                    : "Náhled"}
                            </span>
                            <button
                                onClick={() => updatePreview()}
                        ...
                        <div
                            style={{
                                flex: 1,
                                overflow: "auto",
                                background: previewWidth ? "#1a1a2a" : "#fff",
                                display: "flex",
                                justifyContent: "center",
                            }}
                        >
                            <iframe
                                ...
                                style={{
                                    ...
                                    height: "100%",
                                    flexShrink: 0,
                                    boxShadow: previewWidth
                                        ? "0 0 0 1px rgba(255,255,255,.1), 0 8px 32px rgba(0,0,0,.5)"
                                        : "none",
                                }}
                                title="HTML Preview"
                            />
                        ...
                        {(
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: 600,
                                    color:
                                        rightTab === tab ? D.txtPri : D.txtSec,
                                    display: "flex",
                                    flexDirection: "column",
                                    alignItems: "center",
                                    gap: 2,
                            ...
                            (() => {
                                const html =
                                    activeFile?.type === "html"
                                        ? (contents.get(activeFile.path) ?? "")
                                        : "";
                                const allHtml = activeProject
                                    ? activeProject.files
                                          .filter((f) => f.type === "html")
                                          .map(
                                              (f) => contents.get(f.path) ?? "",
                                          )
                                          .join("\n")
                                    : "";
                                ...
                                const checks = [
                                    {
                                        label: "<title> tag",
                                        ok: /<title>[^<]+<\/title>/i.test(html),
                                        tip: "Každá stránka by měla mít <title>",
                                    },
                                    {
                                        label: "meta description",
                                        ok: /<meta[^>]+name=["']description["'][^>]*>/i.test(
                                            html,
                                        ),
                                        tip: 'Přidej <meta name="description" content="...">',
                                    },
                                    {
                                        label: "lang atribut",
                                        ok: /<html[^>]+lang=/i.test(html),
                                        tip: 'Přidej lang="cs" do <html>',
                                    },
                                    {
                                        label: "meta charset",
                                        ok: /<meta[^>]+charset/i.test(html),
                                        tip: 'Přidej <meta charset="UTF-8">',
                                    },
                                    {
                                        label: "viewport meta",
                                        ok: /<meta[^>]+viewport/i.test(html),
                                        tip: 'Přidej <meta name="viewport" ...>',
                                    },
                                    {
                                        label: "alt u obrázků",
                                        ok: !/<img(?![^>]*alt=)[^>]*>/i.test(
                                        ...
                                        tip: "Všechny <img> by měly mít alt atribut",
                                    },
                                    {
                                        label: "h1 nadpis",
                                        ok: /<h1[^>]*>/i.test(html),
                                        tip: "Stránka by měla mít právě jeden <h1>",
                                    },
                                    {
                                        label: "doctype",
                                        ok: /<!DOCTYPE html>/i.test(html),
                                        tip: "Přidej <!DOCTYPE html> na začátek",
                                    },
                                ];
                                ...
                                for (const tag of pairedTags) {
                                    ...
                                    if (opens !== closes)
                                        tagErrors.push(
                                            `<${tag}>: ${opens}× otevřen, ${closes}× zavřen`,
                                        );
                                }
                                ...
                                return (
                                    <div style={{ padding: "10px 12px" }}>
                                        ...
                                        {!activeFile && (
                                            ...
                                            >
                                                Otevři HTML soubor pro analýzu
                                            </div>
                                        )}
                                    </div>
                                );
                                ...
                                return (
                                    <div style={{ padding: "10px 12px" }}>
                                        ...
                                        <div>
                                            ...
                                            {[
                                                {
                                                    label: "Flexbox centrum",
                                                    code: "display: flex;\njustify-content: center;\nalign-items: center;",
                                                },
                                                {
                                                    label: "CSS Grid 3 sloupce",
                                                    code: "display: grid;\ngrid-template-columns: repeat(3, 1fr);\ngap: 16px;",
                                                },
                                                {
                                                    label: "Box shadow",
                                                    code: "box-shadow: 0 4px 20px rgba(0,0,0,.15);",
                                                },
                                                {
                                                    label: "Gradient pozadí",
                                                    code: "background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);",
                                                },
                                                {
                                                    label: "Přechod (transition)",
                                                    code: "transition: all 0.3s ease;",
                                                },
                                                {
                                                    label: "Border radius zaoblený",
                                                    code: "border-radius: 12px;",
                                                },
                                                {
                                                    label: "Text přetečení",
                                                    code: "overflow: hidden;\ntext-overflow: ellipsis;\nwhite-space: nowrap;",
                                                },
                                                {
                                                    label: "Responsive obrázek",
                                                    code: "max-width: 100%;\nheight: auto;",
                                                },
                                                {
                                                    label: "Sticky header",
                        ...
                        {rightTab === "reference" &&
                            (() => {
                                ...
                                const htmlTags = [
                                    {
                                        tag: "div",
                                        desc: "Blokový kontejner",
                                        ex: '<div class="box">...</div>',
                                    },
                                    {
                                        tag: "span",
                                        desc: "Řádkový kontejner",
                                        ex: '<span style="color:red">text</span>',
                                    },
                                    {
                                        tag: "h1–h6",
                                        desc: "Nadpisy",
                                        ex: "<h1>Hlavní nadpis</h1>",
                                    },
                                    {
                                        tag: "p",
                                        desc: "Odstavec textu",
                                        ex: "<p>Text odstavce.</p>",
                                    },
                                    {
                                        tag: "a",
                                        desc: "Odkaz",
                                        ex: '<a href="https://example.com">Klikni</a>',
                                    },
                                    {
                                        tag: "img",
                                        desc: "Obrázek",
                                        ex: '<img src="foto.jpg" alt="Popis">',
                                    },
                                    {
                                        tag: "ul / ol / li",
                                        desc: "Seznamy",
                                    ...
                                    },
                                    {
                                        tag: "form",
                                        desc: "Formulář",
                                        ex: '<form action="/odeslat" method="POST">\n  <input type="text" name="jmeno">\n  <button>Odeslat</button>\n</form>',
                                    },
                                    {
                                        tag: "input",
                                        desc: "Vstupní pole",
                                        ex: '<input type="text" placeholder="Zadej text">',
                                    },
                                    {
                                        tag: "button",
                                        desc: "Tlačítko",
                                        ex: '<button onclick="klik()">Klikni</button>',
                                    },
                                    {
                                        tag: "header / footer",
                                        desc: "Záhlaví / zápatí stránky",
                                        ex: "<header>\n  <nav>...</nav>\n</header>",
                                    },
                                    {
                                        tag: "nav",
                                    ...
                                    {
                                        tag: "section / article",
                                        desc: "Sekce / článek obsahu",
                                        ex: "<section>\n  <h2>Sekce</h2>\n</section>",
                                    },
                                    {
                                        tag: "main",
                                        desc: "Hlavní obsah stránky",
                                        ex: "<main>\n  <h1>Obsah</h1>\n</main>",
                                    },
                                ];
                                const cssProps = [
                                    ...
                                    {
                                        prop: "background",
                                        desc: "Pozadí elementu",
                                        ex: "background: #f0f0f0;\nbackground: linear-gradient(...)",
                                    },
                                    {
                                        prop: "font-size",
                                        desc: "Velikost písma",
                                    ...
                                    {
                                        prop: "margin / padding",
                                        desc: "Vnější / vnitřní odsazení",
                                        ex: "margin: 16px;\npadding: 8px 16px;",
                                    },
                                    {
                                        prop: "display",
                                        desc: "Způsob zobrazení",
                                        ex: "display: flex;\ndisplay: grid;\ndisplay: none;",
                                    },
                                    {
                                        prop: "flex",
                                        desc: "Flexbox vlastnosti",
                                    ...
                                    {
                                        prop: "overflow",
                                        desc: "Přetečení obsahu",
                                        ex: "overflow: hidden;\noverflow-y: auto;",
                                    },
                                    {
                                        prop: "opacity",
                                        desc: "Průhlednost (0–1)",
                                        ex: "opacity: 0.5;",
                                    },
                                    {
                                        prop: "cursor",
                                    ...
                                    },
                                    {
                                        prop: "transition",
                                        desc: "Animace změn",
                                        ex: "transition: all 0.3s ease;\ntransition: color 0.2s;",
                                    },
                                    {
                                        prop: "box-shadow",
                                        desc: "Stín elementu",
                                        ex: "box-shadow: 0 2px 8px rgba(0,0,0,0.2);",
                                    },
                                    {
                                        prop: "z-index",
                                        desc: "Vrstvení elementů",
                                        ex: "position: relative;\nz-index: 10;",
                                    },
                                ];
                                ...
                                const filteredTags = htmlTags.filter(
                                    (t) =>
                                        ...
                                        t.tag
                                            .toLowerCase()
                                            .includes(
                                                refSearch.toLowerCase(),
                                            ) ||
                                        t.desc
                                            .toLowerCase()
                                            .includes(refSearch.toLowerCase()),
                                );
                                const filteredCss = cssProps.filter(
                                    (t) =>
                                        !refSearch ||
                                        t.prop
                                            .toLowerCase()
