...
export default function StudentTestsClient({ tests, attempts, studentId: _ }: { tests: any[]; attempts: any[]; studentId: string }) {
  ...
  const STATUS_INFO: Record<string, { label: string; bg: string; color: string }> = {
    available:   { label: 'Dostupný',      bg: 'color-mix(in srgb, var(--accent) 12%, #fff)', color: 'var(--accent)' },
    in_progress: { label: 'Probíhá',       bg: '#FEF3C7', color: '#92400E' },
    submitted:   { label: 'Odevzdáno',     bg: 'color-mix(in srgb, var(--accent) 15%, #fff)', color: 'var(--accent)' },
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {hiddenCount > 0 && (
            <button onClick={() => setShowHidden(v => !v)}
              ...
              {showHidden ? '← Zpět' : `🗄 Archivováno (${hiddenCount})`}
            </button>
          )}
      ...
      {visibleTests.length === 0 && !showHidden && (
        <div style={{ textAlign: 'center', padding: '48px 20px', color: '#aaa', border: '1px dashed #e5e7eb', borderRadius: 12 }}>
          Zatím nemáš přiřazené žádné testy.
        </div>
      )}
      {visibleTests.length === 0 && showHidden && (
        <div style={{ textAlign: 'center', padding: '48px 20px', color: '#aaa', border: '1px dashed #e5e7eb', borderRadius: 12 }}>
          Žádné archivované testy.
        </div>
      )}
          ...
          return (
            <div key={t.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 10, opacity: isHidden ? .8 : 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#aaa', display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {retakeMode === 'best' && <span>🔁 Nejlepší z {t.max_attempts ?? '∞'} pokusů · použito {completedCount}</span>}
                {retakeMode === 'practice' && <span style={{ color: '#16a34a' }}>📖 Procvičovací režim</span>}
                {t.available_from && <span>Otevírá se: {new Date(t.available_from).toLocaleString('cs-CZ', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>}
                {t.available_until && <span>Uzavírá se: {new Date(t.available_until).toLocaleString('cs-CZ', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>}
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  {st === 'submitted' && retakeMode !== 'practice' && (
                    <div style={{ fontSize: 12, color: latest?.reviewed_at ? '#27500A' : '#888', background: latest?.reviewed_at ? '#EAF3DE' : '#f9fafb', borderRadius: 8, padding: '6px 12px', textAlign: 'center', fontWeight: 500 }}>
                      {latest?.reviewed_at
                        ? <>✓ Ohodnoceno — <strong>{latest.final_score ?? latest.score ?? '?'} / {latest.max_score ?? '?'} bodů</strong></>
                        : '⏳ Zatím neohodnoceno'}
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: 8 }}>
                    {/* Status badge */}
                    <div style={{ flex: canRetakeNow ? 0 : 1, padding: '9px 12px', background: st === 'timed_out' ? '#f3f4f6' : 'color-mix(in srgb, var(--accent) 15%, #fff)', color: st === 'timed_out' ? '#555' : 'var(--accent)', textAlign: 'center', borderRadius: 8, fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap' }}>
