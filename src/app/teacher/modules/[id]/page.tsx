...
export default async function ModuleDetailPage({ params }: { params: any }) {
  ...
  const { data: mod } = await supabase.from('modules').select('*').eq('id', moduleId).eq('teacher_id', (user as any).id).single()
  ...
  const { data: enrollments } = await supabase.from('enrollments').select('student_id, profiles(full_name, email)').eq('module_id', moduleId)
