...
export default function EditAssignmentPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (a) {
        setTitle((a as any).title)
        setType((a as any).type)
        setLessonId((a as any).lesson_id ?? '')
        setHwInstr((a as any).instructions ?? '')
        setDeadline((a as any).deadline ?? '')
        setQuestions((a as any).questions ?? [])
      }
      setLoading(false)
    }
    load()
  ...
  function addOpt(qi: number) {
    setQuestions(prev => prev.map((q, j) => j === qi ? { ...q, opts: [...(q.opts ?? []), ''] } : q))
  }
  function updateOpt(qi: number, oi: number, val: string) {
    setQuestions(prev => prev.map((q, j) => j === qi ? { ...q, opts: (q.opts ?? []).map((o: string, k: number) => k === oi ? val : o) } : q))
  }
  function removeOpt(qi: number, oi: number) {
    setQuestions(prev => prev.map((q, j) => j === qi ? { ...q, opts: (q.opts ?? []).filter((_: string, k: number) => k !== oi) } : q))
  }
  ...
  function addQ() { setQuestions(prev => [...prev, { type: 'closed', q: '', opts: ['', ''], correct: [0], explanation: '' }]) }

  async function save() {
    if (!title.trim()) { setError('Title is required.'); return }
    setSaving(true); setError('')
    const payload: any = {
      title: title.trim(), type, lesson_id: lessonId || null,
      questions: type === 'homework' ? [] : questions,
  ...
  return (
    <div style={{ maxWidth: 600, margin: '32px auto', padding: '0 20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <h1 style={{ fontSize: 20, fontWeight: 600, marginBottom: 20 }}>Edit assignment</h1>
      <Card>
        ...
        {type === 'homework' ? (
          <>
            ...
            <textarea style={{ ...inp, height: 80, resize: 'vertical' }} value={hwInstr} onChange={e => setHwInstr(e.target.value)} />
          ...
          <>
            ...
            {questions.map((q: any, qi: number) => (
              <div key={qi} style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb', borderRadius: 10, padding: 12, marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <span style={{ flex: 1, fontSize: 11, fontWeight: 500, color: '#888' }}>Q{qi + 1}</span>
                  <select value={q.type} onChange={e => updateQ(qi, 'type', e.target.value)}
                    style={{ padding: '3px 7px', border: '0.5px solid #e5e7eb', borderRadius: 6, fontSize: 11, fontFamily: 'inherit' }}>
                ...
                {q.type !== 'open' && (
                  <>
                    ...
                    <div style={{ marginTop: 8 }}>
                      <label style={{ fontSize: 10, fontWeight: 500, color: '#666', display: 'block', marginBottom: 3 }}>Explanation (shown when wrong)</label>
                ...
                {q.type === 'open' && (
                  <div style={{ fontSize: 11, color: '#888', padding: '6px 10px', background: '#fff', borderRadius: 7, border: '0.5px solid #e5e7eb' }}>
                    Students type a free-text answer — you grade manually.
                  </div>
                )}
              </div>
            ))}
